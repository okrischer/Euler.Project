...
So we have to constrain the generated candidates from this \emph{exhaustive search} to be \emph{good} candidates.
...
Thus, we are searching for a 6-digit number, having a `9' in the first place (most significant digit). \\
The smallest of these numbers is 900900, beeing the product $910 \times 990$.
Constraining the generating function, we only generate a list of all products from the intervals $[910, 999]$ and $[990, 999]$ and then filter out the ones that are palindromes.
...
\end{code}
This solution is faster than the exhaustive search: we only generated $90 \cdot 10 = 900$ possible solutions.
On the other hand, we made the (possibly) wrong assumption that there will be a solution in the interval $[900900, 999999]$, which is actually hard to prove.
However, having another solution from the last section, we can convince ourself that both solutions are probably correct.
